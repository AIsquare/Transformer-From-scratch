...
def get_all_sentences(ds, lang):
    for item in ds:
        yield item['translation'][lang] # pairs
...
def get_ds(config):
    ...
    ds_raw = load_dataset(f"{config['datasource']}", f"{config['lang_src']}-{config['lang_tgt']}", split='train[:1000]')
...
def train_model(config):
    ...
    #print(sum(model.numel() for p in model.parameters()) / 1e6, 'M parameters')
