...
class InputEmbeddings(nn.Module):
    # d_model is dimension of the model
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model:int, seq_len: int, dropout: float)-> None:
        ...
        #sin for even position & cos for odd position
...
class EncoderBlock(nn.Module):
    def __init__(self, features: int, self_attention_block: MultiHeadAttention, feed_forward_block: FeedForwardBlock, dropout: float)-> None:
        ...
        self.residual_connection = nn.ModuleList([ResidualConnection(features,dropout) for _ in range(2)]) # creating 2 residual connection for the Encoder blcok chekc forward connection

    def forward(self,x, src_mask):
        # this is one is query, key, value block, where we are utilising multiheaded attention function
        x = self.residual_connection[0](x, lambda x: self.self_attention_block(x,x,x, src_mask))
        #here we are adding residual function to feedforward
        x = self.residual_connection[1](x, self.feed_forward_block)
        return x
#Defining n Encoder since we can have many encoder
...
class DecoderBlock(nn.Module):
    def __init__(self, features: int, self_attention_block: MultiHeadAttention, cross_attention_block: MultiHeadAttention, feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        ...
        self.feed_forward_block = feed_forward_block
        self.residual_connections = nn.ModuleList([ResidualConnection(features,dropout) for _ in range(3)]) #see architecture why we need 3 res connections
...
class ProjectionLayer(nn.Module):
    # projecting multiheaded attention embedding into the position of vocabulary
    def __init__(self, d_model:int, vocab_size:int):
        super().__init__()
